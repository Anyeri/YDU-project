	...
	public List<C_Info> classStudentInfoList(Pre_C_Info pre_C_Info) {
		...
		try {
			...
			for(C_Info x : getCINum) {
				...
				// 출석율 % 노출용(둘째 자리에서 반올림)
				BigDecimal acdBd = new BigDecimal(attendClassDay);
				BigDecimal tcdBd = new BigDecimal(totalClassDay);
				BigDecimal atdPercent = (acdBd.divide(tcdBd, 1, BigDecimal.ROUND_HALF_UP));
//				double attendPercent = (double) Math.ceil((attendClassDay / totalClassDay * 100)*10 / 10);
//				System.out.println("attendPercent ::: "+attendPercent);
				double attendPercent = atdPercent.doubleValue()*100;
				System.out.println("출석율 ::: "+attendPercent);
				// 출석점수 환산(출석율 * 0.2)
				double a_score = Double.parseDouble(String.format("%.1f", attendPercent*0.2));
				...
				classStudentInfo.setAttendPercent(attendPercent);
				classStudentInfo.setA_score(a_score);
				session.update("inAScore", classStudentInfo);
				int grade =  classStudentInfo.getM_score() + classStudentInfo.getF_score() + (int) classStudentInfo.getA_score();
...
//					System.out.println("attendPercent ::: "+attendPercent);
					...
					myApplyClassInfo.setAttendPercent(attendPercent);
