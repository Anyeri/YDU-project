...
function backUpd() {
   if(confirm("게시글을 수정 하시겠습니까?")==true){
      ...
      location.href="classNotiUpdateForm?b_type="+b_type+"&b_num="+b_num;
   }
}
//게시물 삭제하기 버튼
   ...
   if(confirm("댓글을 수정하시겠습니까?") == true){
      ...
      $.ajax({
         ...
         success : function(data){
            ...
            if(result == 'classCommUpdateOK'){
               alert("댓글 수정이 완료되었습니다!");
               location.reload();
            }else if(result == 'classCommUpdateError'){
...
function classCommWrite(){
   ...
   $.ajax({
      ...
      success : function(data){
         var result = $.trim(data);
         if(result == 'classCommWriteOK'){
            alert("댓글이 작성되었습니다!");
            location.reload(); 
         } else if(result =='classCommWriteError'){
...
//수업 공지 게시판 댓글 삭제 AJax
function classCommDelete(c_num){
   if(confirm("댓글을 삭제 하시겠습니까?") == true){
      ...
      $.ajax({
         ...
         success : function(data){
            var data = $.trim(data);
            if(data == 'classCommDeleteOK'){
               alert("댓글이 삭제 되었습니다!");
               location.reload();
            }else if(data == 'classCommDeleteError'){
...
function commUpdateForm(c_num){
   ...
   /* htmls의 onclick 에서는 데이터를 가져올때 String 형이면 &quoat; 로 써야한다 ex) &quoat;'+b_content+'&quoat;*/
            ...
            <div class="row mt">
               <div class="col-md-12">
                  <h3 style="margin: 18px; color: #4B89DC;">
                     &emsp;<i class="fa fa-bullhorn"></i> <strong>수업 공지사항</strong>
                  </h3>
                  <div class="content-panel" style="width: 1250px; margin: 30px;">
                     ...
                     <!-- 댓글 목록 폼  -->
                     <c:if test="${clSize > 0}">
                        <c:forEach var="classCommentList" items="${classCommentList }">
                        <input type="hidden" id="c_num${classCommentList.c_num }" name="c_num" value="${classCommentList.c_num }">
                         <div id="id${classCommentList.c_num}">
                         <input type="hidden" id="c_w_date" name="c_w_date" value="${classCommentList.c_w_date}">
                           <table>
                              <tr>
                     ...
                     <!-- 댓글 작성폼 -->
                      <table>
                        ...
                        <td width="165px" style="text-align: center">
                           <button type="button" class="btn btn-info" onclick="classCommWrite()" style="margin-left: 15px;">작성하기</button>
                        </td>
                        </tr>
                     </table>
                     <hr>
