...
<script type="text/javascript">
   //전체 공지 게시판 삭제 Ajax
   function notiDelete(b_num, b_type) {
                           ...
                           <c:forEach var="list" items="${boardList}" varStatus="status">
                              <tbody>
                                 <tr>
                                    <td>${list.b_num }</td>
                                    <td>
                                       <a href="notiContent?b_num=${list.b_num }&b_type=${list.b_type}">${list.b_title }
                                       </a>
                                    ...
                                    <td>
                                       <c:if test="${sessionScope.sessionId == list.id}">
                                          ...
                                          <button class="btn btn-success btn-xs">
                                             <a href="notiUpdateForm?b_num=${list.b_num }&b_type=${list.b_type}" style="color: white;"><i class="fa fa-pencil-square-o"></i></a>
                                          </button>
                                       </c:if>
                                       <c:if test="${sessionScope.sessionId == list.id || sessionScope.sessionAutority == 3 }">
                                          <!-- 삭제하기 버튼 -->
                                          <button class="btn btn-danger btn-xs" onclick="notiDelete(${list.b_num},${list.b_type })">
                        ...
                        <div>
                           <c:if test="${sessionScope.sessionAutority == 3}">
                              <button class="btn btn-info" style="width: 50x; margin: 22px;">
                                 <a href="notiWriteForm?id=${sessionScope.sessionId }&b_type=1" style="color: white;">작성하기
                                 </a>
                              </button>
                           </c:if>
                           <!-- 페이지 네비게이터 -->
                              ...
                              <ul class="pagination justify-content-center mt-5">
                                 ...
                                 <c:if test="${page.currentPage > 1 }">
                                    <li class="page-item"><a class="pagenav_a page-link"
                                       href="notiList?b_type=1&currentPage=${page.currentPage - 1}"
                                 ...
                                 <c:forEach var="i" begin="${page.startPage }"
                                    ...
                                    <li class="page-item"><a class="pagenav_a page-link"
                                       href="notiList?b_type=1&currentPage=${i }">${i }</a></li>
                                 </c:forEach>
                                 <!-- 다음 페이지 -->
                                 <c:choose>
